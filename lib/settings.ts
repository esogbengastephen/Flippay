...
interface PlatformSettings {
  ...
  /** Sell rate for SEND only: 1 SEND = sendToNgnSell NGN. USDC/USDT sell in token_sell_prices table (same as buy). */
...
/**
 * Update sell rate for SEND only (1 SEND = sendToNgnSell NGN). USDC/USDT sell in token_sell_prices table.
 */
export async function updateSellRates(
  ...
  global.__sendSettingsCacheTime = Date.now();
  await saveSettings(newSettings);
  console.log(`[Settings] Sell rate (SEND) updated by ${updatedBy || 'system'}`);
  return { ...newSettings };
}
