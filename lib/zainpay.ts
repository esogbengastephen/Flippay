...
const ZAINPAY_ZAINBOX_NAME = process.env.ZAINPAY_ZAINBOX_NAME?.trim();
...
function findZainpayCodeByNigerianCode(nigerianCode: string, zainpayBanks: { code: string; name: string }[]): string | null {
  try {
    ...
    if (ourName.includes("opay") || nigerianCode === "100022") {
      const opay = zainpayBanks.find((b) => /opay/i.test(b.name || ""));
      if (opay) return opay.code;
    }
  } catch {
    // ignore
  }
...
/**
 * Update Zainbox settings (e.g. disable auto-internal transfer for SVA payouts).
 * PATCH https://api.zainpay.ng/zainbox/update
 * codeName = zainboxCode, name = zainboxName.
 */
export async function updateZainbox(options: {
  codeName: string;
  name: string;
  allowAutoInternalTransfer?: boolean;
}): Promise<{ success: boolean; data?: unknown; error?: string }> {
  if (!ZAINPAY_PUBLIC_KEY) {
    return { success: false, error: "Zainpay not configured (ZAINPAY_PUBLIC_KEY)" };
  }
  const codeName = String(options.codeName ?? "").trim();
  const name = String(options.name ?? "").trim();
  if (!codeName || !name) {
    return { success: false, error: "codeName and name are required" };
  }
  const payload: Record<string, unknown> = { codeName, name };
  if (typeof options.allowAutoInternalTransfer === "boolean") {
    payload.allowAutoInternalTransfer = options.allowAutoInternalTransfer;
  }
  try {
    const res = await fetch(`${ZAINPAY_BASE}zainbox/update`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${ZAINPAY_API_TOKEN}`,
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      const msg = data?.description ?? data?.message ?? data?.error ?? res.statusText ?? "Zainbox update failed";
