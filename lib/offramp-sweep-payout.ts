 ...
 * sweep it to the pool wallet and pay out NGN via Zainpay (bank transfer).
...
import { createTransfer as createTransferZainpay, getAccountBalance as getAccountBalanceZainpay } from "./zainpay";
...
export async function processOneOfframpPayout(row: OfframpRow): Promise<{
  ...
  // CDP-only for Base: use CDP transfer. No local sweep fallback.
  ...
  if (row.deposit_private_key_encrypted) {
    ...
    sweepResult = {
      success: false,
      error:
        "Off-ramp requires CDP configuration. Set CDP_API_KEY_ID, CDP_API_KEY_SECRET, and CDP_WALLET_SECRET.",
    };
  } else {
    ...
    if (norm(cdpDepositAddress) !== norm(depositAddressHex)) {
      sweepResult = {
        success: false,
        error:
          "This deposit address is not a CDP wallet. Please cancel and start a new transaction to get a CDP Smart Wallet.",
      };
    } else {
  ...
  // 2.5) NGN float check — never initiate transfer if platform balance is insufficient (Zainpay)
  let ngnBalance = 0;
  try {
    const balanceResult = await getAccountBalanceZainpay();
    ...
    console.warn("[Offramp Payout] Could not fetch Zainpay balance:", (e as Error).message);
  }
  if (ngnBalance > 0 && ngnAmount > ngnBalance) {
    await supabaseAdmin
      ...
      .update({
        status: "failed",
        error_message: `Insufficient float balance. Need ₦${ngnAmount}, available ₦${ngnBalance}. Top up Zainpay NGN balance.`,
  ...
  // 3) NGN transfer via Zainpay
  const reference = `OFFRAMP-${transaction_id}-${Date.now()}`;
  const transferResult = await createTransferZainpay({
  ...
  if (!transferResult.success) {
    await supabaseAdmin
      ...
      .update({
        status: "failed",
        error_message: `Zainpay transfer failed: ${transferResult.error}`,
        swap_tx_hash: sweepResult.txHash,
        token_amount: sendAmount,
        ngn_amount: ngnAmount,
        updated_at: new Date().toISOString(),
  ...
  // 4) Mark as completed — Zainpay has no webhook; Flutterwave webhook only runs for Flutterwave transfers
  ...
  console.log(`[Offramp Payout] Transfer completed ${transaction_id}: ${sendAmount} SEND → ₦${ngnAmount} (ref=${reference}).`);
  return {
    success: true,
    transactionId: transaction_id,
    sendAmount,
