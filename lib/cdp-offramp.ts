 ...
 *
 * CDP account names must be 2-36 chars, alphanumeric and hyphens only.
 * We hash userId to a short suffix since UUIDs exceed the limit.
 */
...
const CDP_NAME_MAX_LEN = 35;

/**
 * Derive a short, deterministic, CDP-compliant suffix from userId.
 * CDP names: 2-36 chars, alphanumeric + hyphens only.
 */
function toCdpSafeSuffix(userId: string): string {
  const hash = createHash("sha256").update(userId, "utf8").digest("hex");
  return hash.slice(0, CDP_NAME_MAX_LEN - OFFRAMP_OWNER_NAME_PREFIX.length);
}
...
export async function getOrCreateOfframpSmartWalletAddress(
  ...
  const suffix = toCdpSafeSuffix(userId);
  const ownerName = `${OFFRAMP_OWNER_NAME_PREFIX}${suffix}`;
  const smartName = `${OFFRAMP_SMART_NAME_PREFIX}${suffix}`;
