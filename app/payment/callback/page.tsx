  ...
  useEffect(() => {
    ...
    async function verifyPayment(attemptNumber = 1) {
      ...
      try {
        ...
        const searchUrl = getApiUrl(`/api/transactions/find-by-txref?txRef=${encodeURIComponent(txRef)}${userId ? `&userId=${encodeURIComponent(userId)}` : ""}`);
        ...
        if (data.success && data.exists && data.transactionId) {
          ...
          const detailResponse = await fetch(getApiUrl(`/api/transactions/create-id?transactionId=${encodeURIComponent(data.transactionId)}`));
        ...
        if (!data.success || !data.exists) {
          ...
          try {
            const debugResponse = await fetch(getApiUrl(`/api/debug/transactions-by-txref?txRef=${encodeURIComponent(txRef)}`));
